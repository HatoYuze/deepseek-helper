...
import com.github.hatoyuze.protocol.api.impl.DeepseekResponsesApiImpl
import com.github.hatoyuze.protocol.api.impl.DeepseekStandardApiImpl
...
internal class DeepseekCore(
    ...
    val backend: DeepseekApiBackend = backend ?: when (api) {
        DeepseekApi.STANDARD -> DeepseekStandardApiImpl(apiKey, beta = enableBeta)
        DeepseekApi.RESPONSES -> DeepseekResponsesApiImpl(apiKey)
    }
    ...
    /** 当前活动流的收集协程，供 [cancelStream] 中止底层请求 */
    @Volatile
     ...
     * 下一次流式调用会自动重置取消状态，调用方的 `collect` 可能抛出 [kotlinx.coroutines.CancellationException]。
     */
    fun cancelStream() {
        isCancelled = true
    ...
    /** 包装流式对话的公共骨架：复位取消标志并注册当前收集协程 */
    fun streamFlow(body: suspend FlowCollector<ChatChunk>.() -> Unit): Flow<ChatChunk> = flow {
        isCancelled = false
        val streamJob = currentCoroutineContext()[Job]
        activeStreamJob = streamJob
        try {
            body()
        } finally {
    ...
    try {
        ...
        while (iterations < core.config.maxToolIterations) {
            ...
            core.backend.completions(
                ...
                if (core.isCancelled) return@collect
                when (chunk) {
