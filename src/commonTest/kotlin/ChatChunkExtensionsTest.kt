...
class ChatChunkExtensionsTest {
    ...
    @Test
    fun `collectResponse accumulates content and thinking`() = runBlocking {
    ...
    @Test
    fun `collectResponse captures tool calls`() = runBlocking {
        val chunks: Flow<ChatChunk> = flowOf(
    ...
    @Test
    fun `collectResponse returns null thinkingContent when no reasoning`() = runBlocking {
        val chunks: Flow<ChatChunk> = flowOf(
    ...
    @Test
    fun `collectResponse defaults usage to zero when no Done chunk`() = runBlocking {
        val chunks: Flow<ChatChunk> = flowOf(
    ...
    @Test
    fun `onThinking triggers for reasoning content`() = runBlocking {
    ...
    @Test
    fun `onContent triggers for regular content`() = runBlocking {
    ...
    @Test
    fun `onContent skips empty strings`() = runBlocking {
        val contentParts = mutableListOf<String>()
        val chunks: Flow<ChatChunk> = flowOf(
            ChatChunk.ContentDelta(content = "", reasoningContent = null),
    ...
    @Test
    fun `onToolCall triggers for tool requests`() = runBlocking {
